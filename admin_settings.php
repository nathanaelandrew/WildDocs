...
// Result flags — shown via inline modal, not PHP alert/flash
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_account') {
    if (trim($_POST['delete_confirm'] ?? '') === 'DELETE') {
        ...
        $deleteError = "Please type 'DELETE' exactly to confirm.";
    }
}
...
<head>
  ...
  <style>
    ...
    .badge-danger { background:#EF4444; color:#fff; padding:2px 8px; border-radius:4px; font-size:.7rem; font-weight:700; }
    /* Result modal colour variants */
    ...
    <div class="dashboard-page">
      ...
      <div class="page-title-row">
        <div>
          ...
          <p>Configure notifications, update security, or manage your account.</p>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1.2fr;gap:24px;align-items:start">
        ...
        <div class="card">
          ...
          <div class="card__body">
            <p style="margin-bottom:20px;font-size:.85rem;color:var(--text-muted)">
              Configure your email and dashboard alerts.
            </p>
        ...
        <div>
          ...
          <div class="card danger-card">
            ...
            <div class="card__body">
              ...
              <p style="font-size:.82rem;color:#7F1D1D;margin-bottom:15px">
                Once deleted, all your access logs and profile data will be purged. This cannot be undone.
              </p>
              <button class="btn btn-danger" onclick="openModal('deleteModal')">Delete My Account</button>
            </div>
          </div>
        </div>
...
<div class="modal-overlay" id="pwConfirmModal">
  <div class="modal" style="max-width:400px;text-align:center">
    ...
    <p style="margin-bottom:24px;color:var(--text-muted);font-size:.88rem">
      Are you sure you want to update your admin password?
...
<!-- Delete error modal — auto-opens if deletion failed -->
...
// ── Password change: intercept submit → show confirm modal first ──────
function interceptPwSubmit(e) {
    e.preventDefault();

    // Client-side quick validation before showing confirm
...
// ── Auto-open result modal if PHP returned a password result ──────────
<?php if ($pwResult): ?>

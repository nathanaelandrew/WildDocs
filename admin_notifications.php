...
// ── Auto-delete notifications (requests rows) older than 7 days that are already viewed ──
// FIX: Wrapped interval duration in single quotes and swapped integer 1 to TRUE
$pdo->exec("
...
$filterTab  = $_GET['filter'] ?? 'all';   // all | unread | read
...
// FIX: Swapped integer matches (= 0, = 1) for real PostgreSQL boolean state tokens
if ($filterTab === 'unread') { $countSql .= " WHERE r.is_viewed = FALSE"; }
elseif ($filterTab === 'read') { $countSql .= " WHERE r.is_viewed = TRUE"; }
...
// ── Data query ────────────────────────────────────────────────────────
$dataSql = "SELECT r.id, r.reference_number, r.is_viewed, r.created_at, r.status,
                   u.first_name, u.last_name, dt.name as document_name
            FROM requests r
            JOIN users u ON r.user_id = u.id
            JOIN document_types dt ON r.document_type_id = dt.id";

// FIX: Swapped integer matches (= 0, = 1) for real PostgreSQL boolean state tokens
if ($filterTab === 'unread') { $dataSql .= " WHERE r.is_viewed = FALSE"; }
elseif ($filterTab === 'read') { $dataSql .= " WHERE r.is_viewed = TRUE"; }

// FIX: Parameterized LIMIT and OFFSET variables via array params to prevent data type coercion failures
$dataSql .= " ORDER BY r.created_at DESC LIMIT ? OFFSET ?";

$ds = $pdo->prepare($dataSql); 
$ds->execute([$perPage, $offset]);
$db_notifs = $ds->fetchAll();

// ── Unread count (always full, not paginated) ─────────────────────────
// FIX: Swapped = 0 for FALSE mapping
$uStmt = $pdo->query("SELECT COUNT(*) FROM requests WHERE is_viewed = FALSE");
...
<div class="app-layout">
  ...
  <main class="main-content" style="background:var(--bg-light);min-height:100vh;padding-bottom:50px">
    <div class="dashboard-page">
      ...
      <!-- Filter tabs (use GET so pagination stays in sync) -->
      <div class="tab-bar">
        <?php
            ...
            <div class="notif-item <?= $isUnread ? 'unread' : '' ?>"
              ...
              <div class="notif-item__body" style="flex:1">
                ...
                <div class="notif-item__desc">
                  ...
                  is at the <u><?= ucfirst($n['status']) ?></u> stage.
                </div>
              </div>
